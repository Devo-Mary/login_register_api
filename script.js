...
async function getData() {
    ...
    row.innerHTML += `<div class="col-md-3">
                            ...
                            <div class="card-body">
                                    ...
                                    <button  class="btn btn-primary float-end " onclick ="addBasket(${item.id})">
...
function addBasket(id) {
    let basketNewProduct = data.find(item =>{
        return id == item.id
    })
    if (basketNewProduct) {
        let productExsist = basketArr.find(item =>{
            return item.id == id
        })
        if (!productExsist) {
            basketNewProduct= {...basketNewProduct, count:1}
            basketArr.push(basketNewProduct)
            counter = basketArr.length
            badge.innerHTML = counter
        }
        else{
            alert('Səbətə bu məhsulu artıq əlavə etmisiniz')
        }
    }
    console.log(counter);
    localStorage.setItem('BasketArray', JSON.stringify(basketArr))
}